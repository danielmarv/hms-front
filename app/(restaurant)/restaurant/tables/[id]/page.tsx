...
export default function EditTablePage() {
  ...
  const { getTable, updateTable, loading } = useTables()
  ...
  useEffect(() => {
    const fetchTable = async () => {
      if (params.id) {
        const table = await getTable(params.id as string)
        if (table) {
          form.reset({
            number: table.number,
    ...
    fetchTable()
  }, [params.id, getTable, form])

  const onSubmit = async (data: TableFormValues) => {
    const result = await updateTable(params.id as string, data)
    if (result) {
    ...
    <div className="container mx-auto p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update table information</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
